...
namespace ColorBlockJam
{
    ...
    return false;
        ...
        public static Vector3 GetOffscreenWorldPos(Vector2 dir,Transform objectcheck)
        {
            // vị trí ngoài màn hình: (-0.1 hoặc 1.1) tùy hướng
            Vector3 viewPos = Camera.main.WorldToViewportPoint(objectcheck.position);
            Vector3 targetView = viewPos;

            if (dir.x < 0) targetView.x = -0.1f;
            else if (dir.x > 0) targetView.x = 1.1f;

            if (dir.y < 0) targetView.y = -0.1f;
            else if (dir.y > 0) targetView.y = 1.1f;

            // giữ nguyên Z
            targetView.z = viewPos.z;

            return Camera.main.ViewportToWorldPoint(targetView);
        }
